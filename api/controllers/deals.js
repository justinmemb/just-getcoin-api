...
var object_id = mongoose.Types.Object_id;
...
exports.dealList = function(request, response) {
    ...
    if(error) {
        ...
        response.json({
            "error": true,
            "message": 'required fields' + error_fields + 'is missing or empty!'
        });
            ...
            users.find({_id: user_id}, function(error, user){
                ...
                if(error) {
                    ...
                    response.json({
                        ...
                        message: message.serverErrorOccured
                    });
                    ...
                    if (user.length) {
                        // User exsits
                        deald.find({$or: [{for_user: user_id}, {to_user: user_id}]}, function(error, result_deals){
                            if (error) {
                                response.json({
                                    error: true,
                                    error_description: error.message,
